...
const ExportInternsPass: React.FC<Props> = ({ interns, name }) => {
  ...
  const handleExport = async () => {
    try {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        sheet.getCell(`D${currentRow}`).value = it.gender;
        sheet.getCell(`E${currentRow}`).value = it.married;
        ...
        if (buf) {
          ...
          sheet.addImage(imgId, {
            tl: { col: 5 + 0.95, row: currentRow - 1 + 0.33 },
...
export default ExportInternsPass;
