...
const ExportListInterns: React.FC<Props> = ({ interns, name }) => {
  ...
  const handleExport = async () => {
    try {
      ...
      title.value = 'Danh Sách Thực Tập Sinh';
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        sheet.getCell(`C${currentRow}`).value = changDateJP(it.birthday);
        sheet.getCell(`D${currentRow}`).value = changDateJP(it.interviewDate);
        sheet.getCell(`E${currentRow}`).value = changDateJP(it.departureDate);
        sheet.getCell(`F${currentRow}`).value = it.address;
        sheet.getCell(`G${currentRow}`).value = it?.tradeUnion?.name;
...
export default ExportListInterns;
