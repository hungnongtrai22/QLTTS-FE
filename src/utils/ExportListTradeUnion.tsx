...
const ExportListTradeUnion: React.FC<Props> = ({ interns, name }) => {
  ...
  const handleExport = async () => {
    try {
      ...
      title.value = 'Danh Sách Nghiệp Đoàn';
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        sheet.getCell(`D${currentRow}`).value = it?.address;
        sheet.getCell(`E${currentRow}`).value = it?.city;
        sheet.getCell(`F${currentRow}`).value = it?.state;
        sheet.getCell(`G${currentRow}`).value = it?.phone;
...
export default ExportListTradeUnion;
