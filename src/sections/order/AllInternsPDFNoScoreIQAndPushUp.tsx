...
// import InternPDFHomeIQ from '../invoice/intern-pdf-home-iq';
import InternPDFHomeIQAndPushUp from '../invoice/intern-pdf-home-iq-push-up';
...
export default function AllInternsPDFIQAndPushUp({ interns }: { interns: any[] }) {
      ...
      {internChunks.map((group, index) => {
        ...
        return (
          <Page key={`home-${index}`} size="A4" orientation="landscape" style={styles.page}>
            <InternPDFHomeIQAndPushUp invoice={group} startIndex={startIndex} />
          </Page>
        );
      })}
