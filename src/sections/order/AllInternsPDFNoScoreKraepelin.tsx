...
import InternPDFHomeNoScoreKraepelin from '../invoice/intern-pdf-home-no-score-kraepelin';
...
export default function AllInternsPDFNoScoreKraepelin({ interns }: { interns: any[] }) {
      ...
      {internChunks.map((group, index) => {
        ...
        return (
          <Page key={`home-${index}`} size="A4" orientation="landscape" style={styles.page}>
            <InternPDFHomeNoScoreKraepelin invoice={group} startIndex={startIndex} />
          </Page>
        );
      })}
