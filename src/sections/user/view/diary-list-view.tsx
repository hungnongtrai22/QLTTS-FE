...
// types
import { IDiaryItem, IUserTableFilters, IUserTableFilterValue } from 'src/types/user';
// _mock
...
import DiaryTableToolbar from '../diary-table-toolbar';
...
export default function DiaryListView() {
  ...
  { id: 'internName', label: t('intern_name'), width: 100 },
  { id: 'status', label: t('status'), width: 120 },
  { id: 'direction', label: t('direction_call'), width: 80 },
  { id: 'startDate', label: t('start_date'), width: 80 },
  { id: 'endDate', label: t('end_date'), width: 80 },
  { id: '', width: 88 },
];
  ...
  const [tableData, setTableData] = useState<IDiaryItem[]>([]);
  ...
  const handleEditRow = useCallback(
    (id: string) => {
      router.push(paths.dashboard.diary.edit(id));
    },
    [router]
  );
  ...
  // }, []);
   ...
   const handleGetTradeUnion = useCallback(async () => {
    ...
    const handleViewRow = useCallback(
      (id: string) => {
        router.push(paths.dashboard.diary.profile(id));
      },
      [router]
    );
      ...
      <Container maxWidth={settings.themeStretch ? false : 'lg'}>
        <CustomBreadcrumbs
          ...
          links={[
            ...
            { name: t('diary') || '', href: paths.dashboard.diary.root },
            { name: t('list') || '' },
          ]}
          action={
            user?.role === "admin" ? <Button
              component={RouterLink}
              href={paths.dashboard.diary.new}
              variant="contained"
